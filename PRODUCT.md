...
- Agent Test: key Gemini disimpan **in-memory server (per sesi admin), tidak dipersist** — hilang saat server restart. `[inferred: pilihan keamanan paling konservatif; sebutkan di UI]`
...
- Agent Test: model Gemini (daftar dari API `models:list` saat key disimpan; fallback presets gemini-2.5-flash / 2.5-pro / 2.0-flash), streaming SSE, function calling memakai tool yang sama seperti MCP, max 12 iterasi tool per putaran, history per sesi admin.
